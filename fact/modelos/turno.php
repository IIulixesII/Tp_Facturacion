<?php
require_once __DIR__ . '/../conexion.php'; // Archivo que debe tener la clase conexion con método conectar()
...
class Turno {
    ...
    public function turnolibre() {
        ...
        $stmt = $conn->prepare("SELECT nombre, numero FROM turno WHERE estado = 'sinatender' ORDER BY numero ASC");
        $stmt->execute();
    ...
    public function turnoAtendido() {
        ...
        $stmt = $conn->prepare("SELECT nombre, numero FROM turno WHERE estado = 'atendiendo' LIMIT 1");
        $stmt->execute();
